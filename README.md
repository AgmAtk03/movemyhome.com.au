...
Useful variables: `{{email_subject}}` `{{company_name}}` `{{customer_name}}` `{{user_email}}` `{{user_phone}}` `{{phone}}` `{{customer_phone}}` `{{pickup_address}}` `{{dropoff_address}}` `{{move_date}}` `{{service_type}}` `{{vehicle}}` `{{total_quote}}` `{{deposit_amount}}` `{{balance_amount}}` `{{inventory}}` `{{route}}` `{{job_details}}` `{{email_kind}}` (`client` or `business` on paid mail; `member` on 5% signup) `{{payment_status}}` (`deposit_paid` on paid mail) `{{stripe_session_id}}` `{{discount_code}}` `{{member_discount}}` `{{quote_subtotal}}`.

Templates can use **Subject `{{email_subject}}`** and a body of `Hi {{customer_name}}`, then `{{job_details}}`, then company sign-off. Paid `{{job_details}}` includes phone, pickup, dropoff, schedule, inventory, quote total, deposit, balance, fuel, and payment status. Member mail sets a 5% subject (never “booking confirmed”) and keeps `MEMBER 5% OFF` / `NEW MEMBER 5% SIGNUP` in `{{job_details}}`.
...
2. Emails the **student** then the **office** via `sendMemberDiscountEmails` — a **separate** sender from `sendPaidBookingEmails`. Transport reuses the two existing template IDs only (`EMAILJS_CLIENT_TEMPLATE_ID` then `EMAILJS_BUSINESS_TEMPLATE_ID`). `email_kind=member` (not `deposit_paid`). Subjects: student `Your 5% student discount code`, office `New member 5% signup` (`{{email_subject}}`). Real fields: `to_email`, `customer_name`, `user_email`, `discount_code`, `reply_to`, plus the offer in `job_details` / `special_instructions`. Unused booking `{{}}` fields are sent **empty** (no fake addresses, dates, vehicles, or quote totals). Never a “booking confirmed” subject.
