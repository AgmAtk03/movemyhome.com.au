...
test('API candidates try same-origin first, then Vercel', () => {
  const urls = apiCandidates('/api/create-checkout-session');
  assert.equal(urls[0], '/api/create-checkout-session');
  assert.ok(urls.includes(`${DEFAULT_API_BASE}/api/create-checkout-session`));
  assert.equal(new Set(urls).size, urls.length);
});

test('fetchApi skips HTML/404 hosts and uses JSON from the next candidate', async () => {
  ...
  try {
    ...
    assert.equal(hits[0], '/api/create-checkout-session');
    assert.ok(hits.some((url) => url.startsWith(DEFAULT_API_BASE)));
  } finally {
